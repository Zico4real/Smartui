...
def _pick_ports(current_ports=None):
    ...
    for i, p in enumerate(KNOWN_CLIENT_FALLBACK_PORTS, 1):
        ...
        print(" [%d] %s%s" % (i, p, tag))
    print(" %s[5]%s Custom port(s)" % (C_YELLOW, C_RESET))
...
def checkuser_api_manager(ports_dict):
    ...
    while True:
        ...
        print("----------------------------------------------------------------")
        print(" %s[1]>%s VIEW ACTIVE SESSIONS (local, admin-only - not networked)" % (C_YELLOW, C_RESET))
        print(" %s[2]>%s CONFIGURE / INSTALL EXPIRY API" % (C_YELLOW, C_RESET))
        print(" %s[3]>%s MANAGE USERS (add / update / remove)" % (C_YELLOW, C_RESET))
        print(" %s[4]>%s CHANGE API PORT(S)" % (C_YELLOW, C_RESET))
        print(" %s[5]>%s VIEW SERVICE LOGS" % (C_YELLOW, C_RESET))
        print(" %s[6]>%s RESTART API SERVICE" % (C_YELLOW, C_RESET))
        print(" [7]> START/STOP API SERVICE [%s]" % status_label)
        print("%s════════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
        print(" %s[0]%s RETURN  %s[8]%s UNINSTALL EXPIRY API" % (C_YELLOW, C_RESET, C_YELLOW, C_RESET))
        print("%s════════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
        ...
        if choice == '0':
            ...
            print("%s════════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
            print(" %s[1]%s Add / update a user" % (C_YELLOW, C_RESET))
            print(" %s[2]%s Remove a user" % (C_YELLOW, C_RESET))
            print(" %s[3]%s List users" % (C_YELLOW, C_RESET))
