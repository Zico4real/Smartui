...
def wireguard_admin_manager(ports_dict):
    ...
    while True:
        ...
        print("----------------------------------------------------------------")
        print(" %s[1]>%s CONFIGURE / INSTALL WIREGUARD (Wizard)" % (C_YELLOW, C_RESET))
        print(" %s[2]>%s MANAGE CLIENT PEERS (Add/Revoke)" % (C_YELLOW, C_RESET))
        print(" %s[3]>%s CHANGE LISTEN PORT" % (C_YELLOW, C_RESET))
        print(" %s[4]>%s VIEW SERVICE STATUS / PEERS" % (C_YELLOW, C_RESET))
        print(" %s[5]>%s RESTART WIREGUARD SERVICE" % (C_YELLOW, C_RESET))
        print(" [6]> START/STOP WIREGUARD SERVICE [%s]" % status_label)
        print("%s════════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
        print(" %s[0]%s RETURN  %s[7]%s UNINSTALL WIREGUARD" % (C_YELLOW, C_RESET, C_YELLOW, C_RESET))
        print("%s════════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
        ...
        if choice == '0':
            ...
            print(" %s[1]%s Add a peer" % (C_YELLOW, C_RESET))
            print(" %s[2]%s Revoke a peer" % (C_YELLOW, C_RESET))
