...
from panel_common import (
    ...
    run_cmd as _run, pick_redirect_target as _pick_target,
)
...
def _restart_stunnel():
    # Same lesson as Dropbear: repeated restart attempts (each config change
    # triggers one, and a failed one triggers a rollback-then-restart too)
    # can exhaust systemd's default restart-rate budget, after which
    # "systemctl restart" is silently refused - the command returns
    # non-zero without the service ever actually attempting to start, so no
    # new log lines appear at all. Confirmed on a real case: journalctl
    # showed nothing since a service "Terminated" days earlier, despite
    # multiple config changes attempted since - clearing this before every
    # restart is what actually lets a new attempt run at all.
    _run("systemctl reset-failed stunnel4")
...
def _ensure_installed():
    if not os.path.exists(STUNNEL_CONF_PATH):
        ...
        # Defense in depth, same reasoning as Dropbear's install path: even
        # though stunnel4 ships disabled by default (ENABLED=0) so a fresh
        # apt install shouldn't auto-start it the way Dropbear's package
        # does, clearing any failed state here means a later restart attempt
        # can never be blocked by something that happened during install.
        _run("systemctl stop stunnel4")
        _run("systemctl reset-failed stunnel4")

    # Confirmed as a real, direct cause of a genuine bug report: this was
    # only ever called from the manual Start/Stop Service menu option, never
    # from Add Port - the actual path almost everyone uses to set Stunnel up
    # in the first place. That meant the service was being STARTED for the
    # current session but never actually ENABLED for boot persistence, so a
    # reboot would leave it down until manually restarted. Calling this here
    # instead, from the one function every setup path already goes through,
    # means it's covered regardless of which specific menu option someone
    # used to get Stunnel running.
    _run("systemctl enable stunnel4")

    # A second, separate cause of the same symptom, confirmed against a real
    # report of the identical behavior elsewhere: stunnel4's own package-
    # provided unit typically ships with no restart policy at all (Restart
    # left at its default of "no") and depends only on network.target, which
    # fires once networking has started initializing, not once it's actually
    # ready. A single transient failure at boot - the network interface not
    # fully up yet, a socket briefly unavailable - then leaves the service
    # down permanently, with nothing to bring it back, until it's started
    # manually. Being enabled was never the missing piece here; enable only
    # controls whether systemd ATTEMPTS to start it at boot, not what
    # happens if that one attempt fails. A systemd drop-in override is the
    # correct way to fix this without touching the package's own unit file
    # directly, which a future apt upgrade would simply overwrite anyway.
    override_dir = "/etc/systemd/system/stunnel4.service.d"
    override_path = "%s/smartui-override.conf" % override_dir
    override_content = (
        "[Unit]\n"
    ...
    needs_override_write = True
    if os.path.exists(override_path):
        with open(override_path) as f:
            needs_override_write = f.read() != override_content
    if needs_override_write:
    ...
    if not os.path.exists(STUNNEL_CONF_PATH) or os.path.getsize(STUNNEL_CONF_PATH) == 0:
        ...
        # An existing config from before this fix would be missing the pid=
        # directive entirely - stunnel4's init script (still what Ubuntu's
        # systemd wrapper invokes) can flatly refuse to start at all without
        # one ("no pid=pidfile specified!"), regardless of which port/target
        # is configured. Repairing this in place rather than only fixing it
        # for brand-new installs going forward.
        with open(STUNNEL_CONF_PATH, "r") as f:
            existing_content = f.read()
        if not re.search(r'^pid\s*=', existing_content, re.MULTILINE):
            lines = existing_content.split('\n')
            for i, line in enumerate(lines):
                if line.startswith('cert ='):
                    lines.insert(i + 1, 'pid = /var/run/stunnel4.pid')
                    break
            else:
                lines.insert(0, 'pid = /var/run/stunnel4.pid')
            with open(STUNNEL_CONF_PATH, "w") as f:
                f.write('\n'.join(lines))
...
def _get_port_targets(ports_dict):
    """Reads (port, target) pairs straight from the live config - the real
    source of truth, since ports_dict only ever tracked port numbers, never
    which backend each one forwards to."""
    if not os.path.exists(STUNNEL_CONF_PATH):
        return []
    with open(STUNNEL_CONF_PATH, "r") as f:
        content = f.read()
    pattern = re.compile(r'\[ssl-tunnel-(\d+)\]\naccept = \d+\nconnect = 127\.0\.0\.1:(\d+)\n')
...
def stunnel_admin_manager(ports_dict):
    ...
    while True:
        # Confirmed as a real, direct cause of the drop-in override never
        # actually reaching a real install: _ensure_installed() (which
        # writes it) was only ever called from three specific actions - Add
        # Port, Edit Config, Start/Stop - never from just opening this menu.
        # An admin who already has Stunnel configured but happens not to
        # touch one of those three specific options after pulling a fix
        # never gets it applied at all, silently. Running it here instead,
        # every time this menu opens, for anyone who already has Stunnel
        # installed, means a fix reaches them the moment they even look at
        # this screen - gated on the conf file already existing so this
        # never installs Stunnel as a side effect of just browsing.
        if os.path.exists(STUNNEL_CONF_PATH):
            _ensure_installed()
        ...
        print(" [1]> ADD PORT")
        print(" [2]> VIEW PORTS")
        print(" [3]> REDIRECT PORT")
        print(" [4]> REMOVE PORT")
        print(" [5]> EDIT STUNNEL CONFIG (nano)")
        print(" [6]> CONFIGURE SSL CERTIFICATE")
        print(" [7]> VIEW STUNNEL LOGS")
        print(" [8]> RESTART STUNNEL SERVICE")
        print(f" [9]> START/STOP STUNNEL SERVICE [{status_label}]")
        print("%s════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
        print(" [0] RETURN  [10] UNINSTALL STUNNEL4")
        print("%s════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
        ...
        if choice == '0':
            ...
            # Checking the LIVE config file, not ports_dict - ports_dict can
            # be cleared (uninstall) or otherwise drift from what's actually
            # on disk, and trusting it here was confirmed as the real cause
            # of a duplicate-section bug: a stale port from before a prior
            # uninstall stayed invisible to this check, so re-adding the
            # same port silently wrote a second, duplicate section that
            # Stunnel couldn't parse at all - unrelated to which port was
            # actually chosen.
            live_ports = [p for p, _ in _get_port_targets(ports_dict)]
            if str(new_port) in live_ports:
                print(f"{C_RED}[✖] Port {new_port} already has a Stunnel service configured - use")
                print(f"    REDIRECT PORT to change it, or REMOVE PORT first.{C_RESET}")
            ...
            if ok:
                tracked = _tracked_ports(ports_dict)
                if str(new_port) not in tracked:
                    tracked.append(str(new_port))
                ports_dict['STUNNEL_PORTS'] = ",".join(tracked)
            ...
            print("%s════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
            pairs = _get_port_targets(ports_dict)
            if not pairs:
                print(f"{C_YELLOW} No ports configured yet - use ADD PORT.{C_RESET}")
            else:
                for port, target in pairs:
                    ...
                    print(f" Port {port:<6} -> 127.0.0.1:{target:<6} {tag}")
            input("\nPress Enter to continue...")
            ...
            if not pairs:
                ...
                input("\nPress Enter to continue...")
                continue
            for idx, (port, target) in enumerate(pairs, 1):
                print(f" [{idx}] Port {port} -> 127.0.0.1:{target}")
            print(" [0] CANCEL")
            ...
            if not pick.isdigit() or not (1 <= int(pick) <= len(pairs)):
                if pick != '0':
                    ...
                    input("\nPress Enter to continue...")
                continue
            target_port_num, old_target = pairs[int(pick) - 1]
            ...
            with open(STUNNEL_CONF_PATH, "r") as f:
                current_content = f.read()
            block_pattern = re.compile(r'\[ssl-tunnel-%s\]\naccept = %s\nconnect = 127\.0\.0\.1:\d+\n' % (target_port_num, target_port_num))
            new_block = '[ssl-tunnel-%s]\naccept = %s\nconnect = 127.0.0.1:%s\n' % (target_port_num, target_port_num, new_target)
            new_content = block_pattern.sub(new_block, current_content)

            ok, msg = _apply_conf_safely(new_content, f"Port {target_port_num} -> {new_target}", int(target_port_num))
            if ok:
                print(f"{C_GREEN}[✔] {msg}{C_RESET}")
            else:
                print(f"{C_RED}[✖] {msg}{C_RESET}")
            ...
            pairs = _get_port_targets(ports_dict)
            if not pairs:
                print(f"{C_YELLOW} No ports configured yet.{C_RESET}")
                input("\nPress Enter to continue...")
                continue
            for idx, (port, target) in enumerate(pairs, 1):
                print(f" [{idx}] Port {port} -> 127.0.0.1:{target}")
            ...
            if not pick.isdigit() or not (1 <= int(pick) <= len(pairs)):
                if pick != '0':
                    ...
                    input("\nPress Enter to continue...")
                continue
            remove_port_num, _ = pairs[int(pick) - 1]

            with open(STUNNEL_CONF_PATH, "r") as f:
                current_content = f.read()
            block_pattern = re.compile(r'\n?\[ssl-tunnel-%s\]\naccept = %s\nconnect = 127\.0\.0\.1:\d+\n' % (remove_port_num, remove_port_num))
            new_content = block_pattern.sub('', current_content)

            remaining = [p for p, t in pairs if p != remove_port_num]
            with open(STUNNEL_CONF_PATH, "w") as f:
                f.write(new_content)
            if _restart_stunnel():
                close_firewall_port(int(remove_port_num), ("tcp",))
                persist_firewall_rules()
                ports_dict['STUNNEL_PORTS'] = ",".join(remaining) if remaining else ""
                print(f"{C_GREEN}[✔] Port {remove_port_num} removed.{C_RESET}")
            else:
                print(f"{C_RED}[✖] Stunnel failed to restart after removing port {remove_port_num} - check")
                print(f"    it manually (systemctl status stunnel4 / journalctl -u stunnel4).{C_RESET}")
            input("\nPress Enter to continue...")
            ...
            if confirm == 'y':
                ...
                # apt-get purge does NOT delete this - it's a file this code
                # wrote directly, not one the .deb package itself ships and
                # tracks, and even for genuinely package-tracked conffiles,
                # dpkg's purge intentionally leaves behind ones it detects
                # were modified after install, to avoid silent data loss.
                # Confirmed as a real, direct cause of a genuine bug report:
                # every port ever added stayed in this file permanently
                # across uninstall/reinstall, invisible to ports_dict (which
                # DOES get cleared above) but still very much live on disk -
                # and Add Port's own pre-check only ever looked at
                # ports_dict, never the file, so it could never see an
                # orphaned section like this to warn about it. The next add
                # would silently write a SECOND, duplicate section with the
                # same name, which Stunnel cannot parse at all - explaining
                # a real "failed to restart Stunnel" report that had nothing
                # to do with the specific port chosen.
                _run(f"rm -f {STUNNEL_CONF_PATH} {STUNNEL_PEM_PATH}")
