...
def stunnel_admin_manager(ports_dict):
    ...
    while True:
        ...
        print("────────────────────────────────────────────────────────────")
        print(" %s[1]>%s ADD SSL / STUNNEL PORT" % (C_YELLOW, C_RESET))
        print(" %s[2]>%s EDIT STUNNEL CONFIG (nano)" % (C_YELLOW, C_RESET))
        print(" %s[3]>%s CONFIGURE SSL CERTIFICATE" % (C_YELLOW, C_RESET))
        print(" %s[4]>%s VIEW STUNNEL LOGS" % (C_YELLOW, C_RESET))
        print(" %s[5]>%s RESTART STUNNEL SERVICE" % (C_YELLOW, C_RESET))
        print(f" {C_YELLOW}[6]>{C_RESET} START/STOP STUNNEL SERVICE [{status_label}]")
        print("%s════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
        print(" %s[0]%s RETURN  %s[7]%s UNINSTALL STUNNEL4" % (C_YELLOW, C_RESET, C_YELLOW, C_RESET))
        print("%s════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
        ...
        if choice == '0':
            ...
            print("%s════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
            print(" %s[1]%s Generate Self-Signed SSL Certificate (.pem)" % (C_YELLOW, C_RESET))
            print(" %s[2]%s Paste Custom Certificate / Key Manually" % (C_YELLOW, C_RESET))
