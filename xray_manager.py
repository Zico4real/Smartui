...
def get_server_ip():
    try:
        ip = subprocess.check_output("curl -4 -s ifconfig.me", shell=True, text=True).strip()
        if ip:
            return ip
    except Exception:
        pass
            ...
            while True:
                ...
                if ssl_choice == "0":
                    ...
                    hook_script = (
                        ...
                        # Confirmed as a real, direct cause of Xray failing to start with
                        # "permission denied" reading the key: Let's Encrypt's own privkey.pem
                        # is tightly restricted (root-only), and cp preserves that by default -
                        # but Xray's systemd service runs as the low-privilege 'nobody' user,
                        # which could never read a key left at those permissions. Every renewal
                        # would silently reproduce this same failure without this fix.
                        f"chgrp nogroup /usr/local/etc/xray/cert/{domain}.key\n"
                        f"chmod 640 /usr/local/etc/xray/cert/{domain}.key\n"
                    ...
                    if result.returncode == 0:
                        ...
                        # Same fix as the renewal hook above, and for the same confirmed
                        # reason: Let's Encrypt's privkey.pem is root-only, cp preserves that,
                        # and Xray's systemd service runs as the low-privilege 'nobody' user -
                        # left unfixed, Xray fails to start with "permission denied" reading
                        # the key on every single attempt to use this cert, regardless of
                        # which protocol, transport, or port the profile actually uses.
                        subprocess.run(f"chgrp nogroup /usr/local/etc/xray/cert/{domain}.key", shell=True)
                        subprocess.run(f"chmod 640 /usr/local/etc/xray/cert/{domain}.key", shell=True)
