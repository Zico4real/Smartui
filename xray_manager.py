...
def check_system_port_in_use(p, needed_protocols=("tcp",)):
    ...
    UDP listeners at all (mKCP, Shadowsocks UDP). Binding is what Xray itself will do,
    so attempting the bind ourselves is the accurate way to detect a real conflict."""
...
def stage_1_installer():
    while True:
        ...
        print("%s════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
        print(" %s[0]%s Return / Exit" % (C_YELLOW, C_RESET))
        print(" %s[1]%s INSTALL V2RAY/XRAY" % (C_YELLOW, C_RESET))
        print("%s════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
        ...
        if choice == "0":
            ...
            while True:
                ...
                print("%s════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
                print(" %s[1]>%s Let's Encrypt" % (C_YELLOW, C_RESET))
                print(" %s[2]>%s Zerossl" % (C_YELLOW, C_RESET))
                print(" %s[3]>%s Mode Manual" % (C_YELLOW, C_RESET))
                print(" %s[4]>%s Url link (.zip)" % (C_YELLOW, C_RESET))
                print("%s════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
...
def add_user():
  ...
  "fragment": {{
    ...
    "interval": "10-20"
        ...
        if not xray_ok:
            print(f"\n{C_RED}Configuration applied but Xray failed to restart.{C_RESET}")
        elif not nginx_ok:
...
def uninstall_script():
    ...
    if confirm == 'y':
        subprocess.run("systemctl stop xray && apt-get purge -y xray", shell=True)
