...
udp_sessions = {}          # session_key (client_addr, instance) -> {"sock": s, "last_active": t}
...
def handle_udp_datagram(sock, data, client_addr, registry):
    ...
    if session is None:
        # Confirmed as a real, necessary fix: the earlier design created a
        # brand-new socket per PACKET, forwarding every packet from a
        # different ephemeral source port. Stateless per-query protocols
        # (dnstt's own wire format) tolerated that fine, but QUIC-based
        # engines (Slipstream) tie session identity to the exact source
        # address:port - every packet after the first looked like an
        # unrelated new client, so the handshake succeeded but no data
        # could ever flow afterward. A stable per-(client, engine) mapping
        # is the real, standard shape of a UDP NAT/proxy.
...
def udp_loop(registry_ref):
    ...
    next_expiry_check = time.monotonic() + 10.0

    while True:
        watch_socks = [sock] + [s["sock"] for s in udp_sessions.values()]
        try:
            readable, _, _ = select.select(watch_socks, [], [], 5.0)
        except Exception:
            continue
        ...
        for rsock in readable:
            if rsock is sock:
                ...
                entry = udp_sessions_by_sock.get(rsock.fileno())
                if entry is None:
                    continue
