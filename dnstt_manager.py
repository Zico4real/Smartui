...
def udp_session_receiver(session_key, backend_sock, main_sock, client_addr):
    ...
    while True:
        try:
            ...
            break  # normal session expiry after idle - not an error, not logged
        except Exception as e:
            print("dns-router: session receiver for %s (%s) ended unexpectedly: %s" % (session_key, client_addr, e))
            break
...
def handle_udp_datagram(sock, data, client_addr, registry):
    ...
    is_new_session_for_log = session_key not in udp_sessions
    if is_new_session_for_log:
