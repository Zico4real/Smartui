...
def udp_session_receiver(client_addr, backend_sock, main_sock):
    """One thread per active client session, reusing the SAME backend
    socket (and therefore the same source port) for the session's entire
    lifetime. Confirmed as a real, necessary fix: the previous design
    created a brand-new socket per packet, forwarding every packet from a
    ...
    with udp_sessions_lock:
        if udp_sessions.get(client_addr, {}).get("sock") is backend_sock:
            udp_sessions.pop(client_addr, None)
    try:
        backend_sock.close()
    except Exception:
        pass
...
def handle_udp_datagram(sock, data, client_addr, registry):
    with udp_sessions_lock:
        session = udp_sessions.get(client_addr)
    ...
    if session is None:
        ...
        with udp_sessions_lock:
            udp_sessions[client_addr] = {"sock": backend_sock, "internal_port": internal_port}
        threading.Thread(target=udp_session_receiver, args=(client_addr, backend_sock, sock), daemon=True).start()
    else:
