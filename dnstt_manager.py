...
udp_sessions = {}          # session_key (client_addr, instance) -> {"sock": s, "last_active": t} - Slipstream ONLY
udp_sessions_by_sock = {}  # backend socket fileno -> (session_key, client_addr)
UDP_SESSION_IDLE_TIMEOUT = 120.0

oneshot_sockets = {}  # backend socket fileno -> socket object - every OTHER engine
oneshot_meta = {}     # backend socket fileno -> (client_addr, created_at)
ONESHOT_TIMEOUT = 5.0
...
def handle_udp_datagram(sock, data, client_addr, registry):
    ...
    if not instance:
        # Logged (not just silently dropped): a real client's packet
        # reaching the router but matching no registered domain is exactly
        # the kind of thing that was previously invisible due to Python's
        # output buffering under systemd - this is the single most useful
        # line for diagnosing "client connects, nothing happens", since it
        # shows definitively whether traffic is arriving at all and what
        # domain it's actually asking for.
        print("dns-router: no matching instance for qname=%r from %s (registered domains: %s)" % (
            qname, client_addr, [e.get("domain") for e in registry.values()]))
        return
    entry = registry[instance]
    internal_port = entry.get("internal_port_udp") or entry.get("internal_port")
    if not internal_port:
        print("dns-router: instance '%s' matched but has no internal_port in its registry entry" % instance)
        return

    # Only Slipstream (QUIC) genuinely needs a persistent backend session -
    # session identity there is tied to the exact source address:port, so
    # a stable per-(client, engine) backend socket is required. Every other
    # engine here (dnstt, vaydns, masterdns, stormdns, cottendns) is
    # stateless at the UDP transport layer and never needed this at all.
    # Confirmed as a severe, real bug applying the persistent-session
    # design to ALL engines: real DNS client behavior uses a fresh source
    # port on every single query (confirmed directly from a live router
    # log - a genuinely new source port on every packet, dozens per
    # second), so every query looked like a brand-new session under the
    # old blanket design. Each one held a file descriptor open for up to
    # 120 idle seconds, and under real browsing load (which needs many
    # rapid queries to move any data at all) this exhausted the process's
    # file descriptor limit outright and crashed the entire UDP loop -
    # "OSError: [Errno 24] Too many open files" - taking every engine down
    # at once, not just the one being tested.
    if entry.get("mode") == "slipstream":
        ...
        if session is None:
            ...
            session = {"sock": backend_sock, "last_active": time.monotonic()}
            udp_sessions[session_key] = session
            udp_sessions_by_sock[backend_sock.fileno()] = (session_key, client_addr)
            print("dns-router: new session for instance '%s' from %s -> 127.0.0.1:%s" % (instance, client_addr, internal_port))
        else:
            session["last_active"] = time.monotonic()
        try:
            session["sock"].sendto(data, ("127.0.0.1", internal_port))
        except Exception as e:
            print("dns-router: failed to forward packet to instance '%s' (127.0.0.1:%s): %s" % (instance, internal_port, e))
            _close_udp_session(session_key)
        return

    # Every other engine: a fresh, non-blocking backend socket per packet,
    # closed the moment its one response comes back (or on timeout/error) -
    # never lingers, so it can never accumulate the way a 120-second-idle
    # persistent session could. This is the same shape that worked fine
    # for dnstt before any of the session-handling changes above existed,
    # just now driven by select() instead of a blocking synchronous
    # sendto+recvfrom, so it doesn't reintroduce the per-packet-thread
    # throughput problem either.
    backend_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    backend_sock.setblocking(False)
    try:
        ...
        try:
            ...
            pass
        return
    oneshot_sockets[backend_sock.fileno()] = backend_sock
    oneshot_meta[backend_sock.fileno()] = (client_addr, time.monotonic())


def _close_udp_session(session_key):
    session = udp_sessions.pop(session_key, None)
    if session is None:
        return
    backend_sock = session["sock"]
    udp_sessions_by_sock.pop(backend_sock.fileno(), None)
    try:
        backend_sock.close()
    except Exception:
        pass
...
def _close_oneshot(fd):
    sock_obj = oneshot_sockets.pop(fd, None)
    oneshot_meta.pop(fd, None)
...
def udp_loop(registry_ref):
    """Single-threaded, select()-based event loop for ALL UDP traffic -
    both incoming client queries and backend responses - rather than
    spawning a new OS thread per packet and per session. Confirmed as a
    real, severe throughput regression in the earlier per-packet-thread
    design: DNS tunneling protocols need hundreds to thousands of small
    packets per second to move any meaningful amount of data (loading even
    a simple page), and Python thread creation/destruction at that rate,
    compounded by GIL contention across many concurrent threads, was the
    actual cause of a genuine "connects but painfully slow" report. This
    is the same select()-based shape already proven working in
    icmp_manager.py's own server script - one thread, no per-packet
    spawning, handling many concurrent sessions efficiently."""
    import select
    ...
    next_expiry_check = time.monotonic() + 2.0

    while True:
        watch_socks = [sock] + [s["sock"] for s in udp_sessions.values()] + list(oneshot_sockets.values())
        try:
            readable, _, _ = select.select(watch_socks, [], [], 2.0)
        except Exception:
            continue
        ...
        for rsock in readable:
            if rsock is sock:
                try:
                    ...
                    continue
                handle_udp_datagram(sock, data, client_addr, registry_ref[0])
                continue

            fd = rsock.fileno()
            if fd in oneshot_sockets:
                client_addr, _ = oneshot_meta.get(fd, (None, None))
                try:
                    response, _ = rsock.recvfrom(65535)
            ...
            entry = udp_sessions_by_sock.get(fd)
            if entry is None:
                continue
            session_key, client_addr = entry
            try:
                response, _ = rsock.recvfrom(65535)
                sock.sendto(response, client_addr)
                session = udp_sessions.get(session_key)
                ...
                print("dns-router: session for %s (%s) ended unexpectedly: %s" % (session_key, client_addr, e))
                _close_udp_session(session_key)

        now = time.monotonic()
        if now >= next_expiry_check:
            next_expiry_check = now + 2.0
            for session_key in list(udp_sessions.keys()):
                session = udp_sessions.get(session_key)
                if session and now - session["last_active"] > UDP_SESSION_IDLE_TIMEOUT:
                    _close_udp_session(session_key)
            for fd in list(oneshot_meta.keys()):
                _, created_at = oneshot_meta.get(fd, (None, now))
                if now - created_at > ONESHOT_TIMEOUT:
                    _close_oneshot(fd)
...
def handle_tcp_connection(conn, registry):
    ...
    try:
        ...
        instance = find_matching_instance(qname, registry)
        if not instance:
            print("dns-router (TCP): no matching instance for qname=%r (registered domains: %s)" % (
                qname, [e.get("domain") for e in registry.values()]))
            return

        internal_port = registry[instance].get("internal_port_tcp") or registry[instance].get("internal_port")
        if not internal_port:
            print("dns-router (TCP): instance '%s' matched but has no internal_port_tcp/internal_port" % instance)
            return
...
def _ensure_router_deployed():
    ...
    service_needs_rewrite = service_created
    if not service_created:
        with open(DNS_ROUTER_SERVICE_PATH) as f:
            existing_service_content = f.read()
        # Confirmed as a major, real bug: Python block-buffers stdout by
        # default whenever it isn't connected to a terminal - exactly the
        # case running under systemd. This script runs an infinite loop and
        # is killed (not exited cleanly) on every restart/stop, so anything
        # it ever printed - including its own unconditional "listening on
        # 0.0.0.0:53" startup message - was sitting in a buffer that never
        # got flushed to the journal at all. journalctl showing nothing was
        # never evidence that nothing was going wrong; it meant NOTHING the
        # script ever did was visible, full stop. Retrofitting existing
        # installs (not just fresh ones) since this masks real diagnosis
        # for anyone who installed before this fix existed.
        if "PYTHONUNBUFFERED" not in existing_service_content:
            service_needs_rewrite = True

    if service_needs_rewrite:
...
"""
    ...
    if needs_write or service_needs_rewrite:
...
def _install_dnstt(custom_port, domain_ns, target_port, fallback_addr, mtu=None):
    ...
    _run("apt-get update && apt-get install -y golang git iptables")
    install_iptables_persistent()
...
def _multi_engine_menu(ports_dict):
    ...
    while True:
        ...
        print(" [3] Remove instance       [4] Restart all instances")
        print(" [0] Back")
        print("%s════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
        ...
        if choice == '0':
            ...
            clear_screen()
            print("%s════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
            print("%s                  RESTART ALL INSTANCES                      %s" % (C_BOLD, C_RESET))
            print("%s════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
            if not registry:
                print(" No instances to restart.")
            else:
                confirm = input(f" Restart all {len(registry)} instance(s)? (y/n): ").strip().lower()
                if confirm == 'y':
                    for inst_name, entry in registry.items():
                        # Same svc naming this menu's own status display
                        # already uses (line ~1280 above), so an instance
                        # shown as [ON]/[OFF] here restarts the exact
                        # service that status reflects.
                        svc = f"dnstt-{inst_name}" if entry['mode'] != 'masterdns' or True else inst_name
                        ok = _run(f"systemctl restart {svc}").returncode == 0
                        if ok:
                            print(f"{C_GREEN}[✔] {inst_name} ({entry['mode']}) restarted.{C_RESET}")
                        else:
                            print(f"{C_RED}[✖] {inst_name} ({entry['mode']}) failed to restart - check "
                                  f"'systemctl status {svc}' for details.{C_RESET}")
                    # The router itself sits in front of every instance on
                    # the shared port 53 - restarting it too so a
                    # bulk-restart genuinely clears any stuck state at
                    # that layer as well, not just the instances behind it.
                    router_ok = _run("systemctl restart dns-router").returncode == 0
                    if router_ok:
                        print(f"{C_GREEN}[✔] Router restarted.{C_RESET}")
                    else:
                        print(f"{C_RED}[✖] Router failed to restart - check 'systemctl status "
                              f"dns-router' for details.{C_RESET}")
            input("\nPress Enter to continue...")
...
def _save_state_now(ports_dict):
    ...
    the real, direct cause of a genuine bug report: the main loop only ever
    calls save_ports_dict() as its very last line, once a submenu function
    fully returns - so an admin who installs something here and then stays
    inside this module's own screens (e.g. viewing the connection info
    right after install, exactly what the install flow itself invites them
    to do) has that change sitting only in memory. If the session ends
    before they navigate all the way back to the main menu - SSH drops, the
    VPS reboots, the terminal closes - it's silently lost, even though the
    actual service the change describes keeps running fine on disk,
    independent of the panel. Lazy-imported to avoid a circular import,
    since smartui_main.py is what imports this module, not the reverse."""
    try:
    ...
    while True:
        ...
        if not is_installed:
            ...
            if mode == 'masterdns':
                ...
                print(f" ENCRYPTION KEY: {key_content}")
                print("-----------------------------------------------------")
                print(" [🔗] CLIENT SETUP:")
                ...
                print(" [9] > MULTI-ENGINE MODE (run more of these at once)")
                print(" [0] > Back")
                print("%s═════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
            else:
                ...
                if pub_key_content:
                    print(f" PUBLIC KEY: {pub_key_content}")
                print("-----------------------------------------------------")
