...
def handle_udp_datagram(sock, data, client_addr, registry):
    ...
    try:
        ...
        backend_sock.settimeout(UDP_TIMEOUT)
...
def _ensure_router_deployed():
    ...
    if not os.path.exists(DNS_ROUTER_SERVICE_PATH):
...
def _install_vaydns_routed(name, domain_ns, target_port, internal_port, mtu=None, record_type=None, dnstt_compat=False):
...
def _install_slipstream_routed(name, domain_ns, target_port, internal_port):
    ...
    _run("apt-get update && apt-get install -y cargo git")
    _run("cargo install --root /usr/local slipstream-rust 2>/dev/null || true")
    slipstream_bin = SLIPSTREAM_BIN if _binary_ok(SLIPSTREAM_BIN) else "/usr/local/bin/slipstream-rust"
    if not _binary_ok(slipstream_bin):
        print(f"{C_RED}[✖] Could not install Slipstream.{C_RESET}")
        return False

    instance_dir = f"{INSTANCES_DIR}/{name}"
    os.makedirs(instance_dir, exist_ok=True)
    cert_dir = instance_dir
    exec_cmd = (f"{slipstream_bin} server --listen 0.0.0.0:{internal_port} --domain {domain_ns} "
                f"--upstream 127.0.0.1:{target_port} --cert-dir {cert_dir}")
    return _write_routed_service(name, exec_cmd)
...
def _install_vaydns(custom_port, domain_ns, target_port, mtu=None, record_type=None, dnstt_compat=False):
...
def _install_slipstream(custom_port, domain_ns, target_port):
    ...
    if _run("which cargo").returncode != 0:
        print("[i] Installing Rust toolchain...")
...
def _install_masterdns_routed(name, domain_ns, internal_port):
    ...
    ok = _write_routed_service(name, f"/opt/masterdnsvpn/masterdnsvpn --config {instance_dir}/server_config.toml") \
        if os.path.exists("/opt/masterdnsvpn/masterdnsvpn") else False
    return ok, None
...
def _install_stormdns_routed(name, domain_ns, internal_port):
    ...
    bin_path = f"{instance_dir}/StormDNS_Server_Linux_AMD64"
...
def _install_cottendns_routed(name, domain_ns, internal_udp_port, internal_tcp_port):
    ...
    bin_candidates = [f"{instance_dir}/cottendns", f"{instance_dir}/cottendns-server"]
...
def _multi_engine_menu(ports_dict):
    ...
    while True:
        ...
        print("%s════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
        print(" %s[1]%s Add instance          %s[2]%s View all profiles (copy info)" % (C_YELLOW, C_RESET, C_YELLOW, C_RESET))
        print(" %s[3]%s Remove instance       %s[0]%s Back" % (C_YELLOW, C_RESET, C_YELLOW, C_RESET))
        print("%s════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
        ...
        if choice == '0':
            ...
            print("%s════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
            print(" %s[1]%s DNSTT   %s[2]%s Slipstream   %s[3]%s MasterDnsVPN" % (C_YELLOW, C_RESET, C_YELLOW, C_RESET, C_YELLOW, C_RESET))
            print(" %s[4]%s VayDNS  %s[5]%s StormDNS      %s[6]%s CottenDNS" % (C_YELLOW, C_RESET, C_YELLOW, C_RESET, C_YELLOW, C_RESET))
...
def dnstt_admin_manager(ports_dict):
    ...
    while True:
        ...
        if not is_installed:
            ...
            print(" %s[1]%s DNSTT (bamsoftware)" % (C_YELLOW, C_RESET))
            print(" %s[2]%s Slipstream" % (C_YELLOW, C_RESET))
            print(" %s[3]%s MasterDnsVPN" % (C_YELLOW, C_RESET))
            print(" %s[4]%s VayDNS" % (C_YELLOW, C_RESET))
            print(" %s[5]%s StormDNS" % (C_YELLOW, C_RESET))
            print(" %s[6]%s CottenDNS" % (C_YELLOW, C_RESET))
            print(" %s[7]%s MULTI-ENGINE MODE" % (C_YELLOW, C_RESET))
            print("%s════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
            ...
            if mode == 'masterdns':
                ...
                print("%s═════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
                print(" %s[4]%s > VIEW SERVICE STATUS" % (C_YELLOW, C_RESET))
                print(" %s[5]%s > EDIT CONFIG (server_config.toml)" % (C_YELLOW, C_RESET))
                print(" %s[6]%s > RESTART SERVICE" % (C_YELLOW, C_RESET))
                print(" %s[7]%s > START / STOP SERVICE" % (C_YELLOW, C_RESET))
                print("-----------------------------------------------------")
                print(" %s[8]%s > UNINSTALL (official uninstaller)" % (C_YELLOW, C_RESET))
                print(" %s[0]%s > Back" % (C_YELLOW, C_RESET))
                print("%s═════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
                ...
                print(" %s[7]%s > START / STOP SERVICE" % (C_YELLOW, C_RESET))
                print("-----------------------------------------------------")
                print(" %s[8]%s > UNINSTALL SLOWDNS" % (C_YELLOW, C_RESET))
                print(" %s[M]%s > MULTI-ENGINE MODE (run more of these at once)" % (C_YELLOW, C_RESET))
                print(" %s[0]%s > Back" % (C_YELLOW, C_RESET))
                print("%s═════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
            else:
                ...
                for label, cmd in client_cmds.items():
                    ...
                    print(f"        {cmd}")
                print("%s═════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
                print(" %s[1]%s > MODIFY DATA TRAFFIC PORT (REDIRECTION)" % (C_YELLOW, C_RESET))
                print(" %s[2]%s > MODIFY NS DOMAIN OR IP" % (C_YELLOW, C_RESET))
                if mode in ('dnstt', 'vaydns'):
                    print(" %s[3]%s > GENERATE NEW KEYS (restarts the service)" % (C_YELLOW, C_RESET))
                print("-----------------------------------------------------")
                print(" %s[4]%s > VIEW SERVICE STATUS" % (C_YELLOW, C_RESET))
                print(" %s[5]%s > EDIT SERVICE" % (C_YELLOW, C_RESET))
                print(" %s[6]%s > RESTART SERVICE" % (C_YELLOW, C_RESET))
                print(" %s[7]%s > START / STOP SERVICE" % (C_YELLOW, C_RESET))
                print("-----------------------------------------------------")
                print(" %s[8]%s > UNINSTALL SLOWDNS" % (C_YELLOW, C_RESET))
