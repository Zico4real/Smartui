    ...
    while True:
        ...
        if choice == '0':
            ...
            if os.path.exists(DROPBEAR_CONF_PATH):
                ...
                # Must check for an ACTIVE (uncommented) directive specifically -
                # a plain substring check matches "#DROPBEAR_PORT=22" too (the
                # commented-out default every fresh install ships with), but the
                # replacement regex below only matches lines that actually START
                # with "DROPBEAR_PORT=". Confirmed on a real VPS: the substring
                # check said a line existed, the regex silently matched nothing
                # against the commented line, and the file was never actually
                # changed at all - dropbear kept defaulting to port 22 forever,
                # regardless of what was entered here.
