    ...
    while True:
        ...
        if choice == '0':
            ...
            if not os.path.exists(DROPBEAR_CONF_PATH):
                ...
                # The package's own post-install immediately starts dropbear with
                # its default template config (port 22) - on most VPS setups
                # OpenSSH is already there, so this collides, crash-loops, and
                # burns through systemd's default 5-attempts-per-10s restart
                # budget before we ever get a chance to write the real port
                # below. Stopping it here, before that loop can exhaust the
                # budget, is what actually prevents the failure - confirmed
                # against a real case where this exact conflict (dropbear vs.
                # OpenSSH, both defaulting to 22) produced precisely this.
