...
def _apply_bundled_dropbear_binary():
    ...
    # A plain in-place `cp` here fails with "Text file busy" whenever the
    # service is currently running (the kernel won't let you overwrite an
    # executable file that's actively mapped by a live process) -
    # confirmed as a real, silent failure: _run()'s captured output meant
    # this error was previously invisible, the function just quietly did
    # nothing every time. Copying to a temp name first, then renaming it
    # into place, sidesteps this entirely - rename() just repoints the
    # directory entry to the new inode rather than overwriting in place,
    # so it succeeds even while dropbear is still running against the old
    # one. The already-running process keeps using its old in-memory code
    # until restarted, so a service restart is still required afterward
    # for the new binary to actually take effect - this only guarantees
    # the file replacement itself won't fail.
    tmp_path = live + ".new"
    result = _run(["cp", "-p", bundled, tmp_path])
    if result.returncode != 0:
        return
    _run(["chmod", "755", tmp_path])
