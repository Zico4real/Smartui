...
def ws_epro_admin_manager(ports_dict):
    ...
    while True:
        ...
        print("----------------------------------------------------------------")
        print(" %s[1]>%s CONFIGURE / INSTALL WS-EPRO" % (C_YELLOW, C_RESET))
        print(" %s[2]>%s MODIFY PORT & TRAFFIC REDIRECTION" % (C_YELLOW, C_RESET))
        print(" %s[3]>%s VIEW SERVICE LOGS" % (C_YELLOW, C_RESET))
        print(" %s[4]>%s RESTART WS-EPRO SERVICE" % (C_YELLOW, C_RESET))
        print(f" {C_YELLOW}[5]>{C_RESET} START/STOP WS-EPRO SERVICE [{status_label}]")
        print("%s════════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
        print(" %s[0]%s RETURN  %s[6]%s UNINSTALL WS-EPRO" % (C_YELLOW, C_RESET, C_YELLOW, C_RESET))
        print("%s════════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
