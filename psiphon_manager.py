    ...
    while True:
        ...
        if choice == '0':
            ...
            if existing_ssh_users:
                # Same fix as Squid's own identical picker elsewhere in this
                # panel, for the same real, confirmed scaling concern:
                # dumping every SSH account into a numbered list is
                # completely unusable for a reseller with hundreds of real
                # customers. Searching by typing part of the username
                # scales to any number of accounts without ever dumping
                # the full list unless there's a genuine need to
                # disambiguate a few close matches.
