...
def filebrowser_admin_manager(ports_dict):
    ...
    while True:
        ...
        print("----------------------------------------------------------------")
        print(" %s[1]>%s CONFIGURE / INSTALL FILEBROWSER (Wizard)" % (C_YELLOW, C_RESET))
        print(" %s[2]>%s CHANGE FILEBROWSER PORT" % (C_YELLOW, C_RESET))
        print(" %s[3]>%s RESET ADMIN PASSWORD" % (C_YELLOW, C_RESET))
        print(" %s[4]>%s VIEW SERVICE LOGS" % (C_YELLOW, C_RESET))
        print(" %s[5]>%s RESTART FILEBROWSER SERVICE" % (C_YELLOW, C_RESET))
        print(" [6]> START/STOP FILEBROWSER SERVICE [%s]" % status_label)
        print("%s════════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
        print(" %s[0]%s RETURN  %s[7]%s UNINSTALL FILEBROWSER" % (C_YELLOW, C_RESET, C_YELLOW, C_RESET))
        print("%s════════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
