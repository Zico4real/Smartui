...
def _build_config(domain, cert_path, key_path, registry, decoy_url, listen_port=443):
    """listen_port: 443 for Nginx owning the public port directly (standalone).
    Any other port means Nginx is chained behind another port-sharing tool
    (MULTIPLEXING/sslh or HAProxy) that forwards matching traffic here over
    loopback - sslh's own "tls" probe only peeks at the handshake to detect
    TLS, then forwards the raw, still-encrypted bytes unmodified, so Nginx
    still needs to terminate TLS itself here regardless of which port it's
    on. The port-80 HTTP->HTTPS redirect only makes sense in standalone
    mode - in chained mode, sslh (or HAProxy) already owns the public
    80/443 and this internal port is loopback-only, so that redirect
    would be unreachable from outside and is skipped entirely."""
    lines = []
    ...
    if listen_port == 443:
        ...
        # Chained mode: loopback-only, since the public port belongs to
        # whatever's forwarding traffic here.
        lines.append("    listen 127.0.0.1:%s ssl;" % listen_port)
    lines.append("    server_name %s;" % domain)
...
def _write_full_config(domain, cert_path, key_path, registry, decoy_url, listen_port=443):
    content = _build_config(domain, cert_path, key_path, registry, decoy_url, listen_port)
...
def _apply_safely(domain, cert_path, key_path, registry, decoy_url, description, listen_port=443):
    original = None
    if os.path.exists(NGINX_CONF_PATH):
        with open(NGINX_CONF_PATH, "r") as f:
            original = f.read()

    _write_full_config(domain, cert_path, key_path, registry, decoy_url, listen_port)
    ...
    if not _restart_nginx():
        if original is not None:
            ...
            # this rollback restart's own outcome was silently ignored -
            # if the OLD config also fails to come back up (e.g. because
            # something else now legitimately holds the port this config
            # used to own), the admin was told the same generic "failed
            # to restart" either way, with no indication nginx might now
            # be down entirely rather than safely back on its previous,
            # working setup.
            if _restart_nginx():
                return False, "%s failed to restart - reverted to the previous config, which is running again. Check 'journalctl -u nginx' for what went wrong with the new one." % description
            else:
                return False, "%s failed to restart, AND reverting to the previous config also failed - nginx is likely not running at all right now. Check 'journalctl -u nginx' for both errors." % description
        else:
            # Same fix as the identical bug already found and fixed for
            # Hysteria2/BADVPN/OpenVPN/MULTIPLEXING elsewhere in this
            # panel: on a genuine first-ever attempt (no prior working
            # config to revert to), leaving the broken new config sitting
            # on disk would mean every later attempt's own rollback
            # restores THIS same broken config as its baseline.
            try:
                os.remove(NGINX_CONF_PATH)
            except FileNotFoundError:
                pass
    ...
    if not _wait_for_port_listening(listen_port):
        if original is not None:
            with open(NGINX_CONF_PATH, "w") as f:
                f.write(original)
            # Same accurate reporting as above, for this failure path too.
            if _restart_nginx():
                return False, "%s restarted, but port %s never came up - reverted to the previous config, which is running again." % (description, listen_port)
            else:
                return False, "%s restarted, but port %s never came up - reverting also failed, nginx is likely not running at all right now. Check 'journalctl -u nginx'." % (description, listen_port)
        else:
...
def nginx_admin_manager(ports_dict):
    ...
    while True:
        ...
        if listen_port == 443:
            ...
            print("       (forward matching traffic here from MULTIPLEXING or HAProxy)")
        ...
        if choice == '0':
            ...
            # request: MULTIPLEXING (sslh) and HAProxy both also want to
            # directly own port 443, and only one process can actually
            # bind a port at a time - trying to run Nginx standalone
            # alongside either of those is a guaranteed conflict, not a
            # bug to fix. Chained mode instead has Nginx listen on
            # loopback only, on an internal port the admin picks, so
            # sslh/HAProxy can keep the public port and forward matching
            # traffic here - sslh's own "tls" probe only peeks at the
            # handshake to detect TLS, then forwards the raw, still-
            # encrypted bytes unmodified, so Nginx still does real TLS
            # termination here regardless of which port it's on.
            chain_choice = input(" Will Nginx own port 443 directly (standalone), or run chained\n"
                                  " behind MULTIPLEXING/HAProxy on an internal port instead?\n"
                                  " Enter an internal port to chain (e.g. 8443), or blank for standalone: ").strip()
            if chain_choice:
                try:
            ...
            if listen_port == 443:
                if not is_active and check_system_port_in_use(443, ("tcp",)):
                    ...
                    print("    first (or check if HAProxy/MULTIPLEXING is using it - consider")
                ...
                # resolve it themselves. Only relevant in standalone mode -
                # chained mode never touches port 80 at all.
            ...
            # from sslh/HAProxy over 127.0.0.1, never from outside - no
            # firewall opening needed for it at all.

            new_state = {"domain": new_domain, "cert_path": cert_path, "key_path": key_path,
                         "decoy_url": decoy_url, "listen_port": listen_port}
            ok2, msg = _apply_safely(new_domain, cert_path, key_path, registry, decoy_url, "Initial setup", listen_port)
            if ok2:
                ...
                if listen_port == 443:
                    ...
                    print("    Domain set to %s, listening internally on 127.0.0.1:%s." % (new_domain, listen_port))
                    print("    Now point MULTIPLEXING's TLS backend (or HAProxy's route) at")
                    print("    127.0.0.1:%s, and add your own routes here with option 2." % listen_port)
            else:
            ...
            ok, msg = _apply_safely(domain, state.get("cert_path"), state.get("key_path"), new_registry, decoy_url, "Route for %s" % path, listen_port)
            if ok:
            ...
            ok, msg = _apply_safely(domain, state.get("cert_path"), state.get("key_path"), new_registry, decoy_url, "Removing route for %s" % path_to_remove, listen_port)
            if ok:
                _save_registry(new_registry)
            ...
            ok, msg = _apply_safely(domain, state.get("cert_path"), state.get("key_path"), registry, new_decoy, "Decoy site change", listen_port)
            if ok:
                _save_state(new_state)
