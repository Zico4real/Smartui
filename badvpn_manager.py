...
def badvpn_admin_manager(ports_dict):
    ...
    while True:
        ...
        print("----------------------------------------------------------------")
        print(" %s[1]>%s CONFIGURE / INSTALL BADVPN-UDPGW" % (C_YELLOW, C_RESET))
        print(" %s[2]>%s CHANGE GATEWAY PORT" % (C_YELLOW, C_RESET))
        print(" %s[3]>%s VIEW SERVICE LOGS" % (C_YELLOW, C_RESET))
        print(" %s[4]>%s RESTART BADVPN SERVICE" % (C_YELLOW, C_RESET))
        print(f" {C_YELLOW}[5]>{C_RESET} START/STOP BADVPN SERVICE [{status_label}]")
        print("%s════════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
        print(" %s[0]%s RETURN  %s[6]%s UNINSTALL BADVPN-UDPGW" % (C_YELLOW, C_RESET, C_YELLOW, C_RESET))
        print("%s════════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
