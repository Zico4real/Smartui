...
def _restart_badvpn():
    ...
    # this time.
    #
    # Also enable here, not just restart - confirmed as a real gap: only
    # the install wizard ever called `systemctl enable`, so a service that
    # ended up running through this function alone (restart, or a config
    # change that calls this) could be genuinely active right now but
    # still `disabled`, surviving nothing across a reboot. `restart`
    # starts a disabled service fine without any error - there was no
    # visible sign anything was wrong until the VPS actually rebooted.
    _run("systemctl enable badvpn")
    _run("systemctl reset-failed badvpn")
    r = _run("systemctl restart badvpn")
    if r.returncode != 0:
        _run("systemctl enable badvpn-udpgw")
        _run("systemctl reset-failed badvpn-udpgw")
        r = _run("systemctl restart badvpn-udpgw")
    return r.returncode == 0


def _wait_for_port_listening(port, tries=15, delay=1):
    # The real root cause, found by tracing badvpn's own source directly
    # (udpgw.c): its --listen-addr socket is accepted via BListener/
    # BConnection_Init - badvpn's own stream-oriented, TCP-style "accept a
    # connection" abstractions, the same ones badvpn-tun2socks uses for its
    # own explicitly-TCP SOCKS connections (per its own man page). The
    # "UDP" in udpgw's name refers to the payload it relays, not its own
    # listen socket's transport - that's genuinely TCP. This check
    # previously looked for a UDP socket on this port, which never
    # existed regardless of whether the install actually worked, so every
    # single install was reported as "reverted" even when badvpn-udpgw
    # was running completely correctly. The 15-try/15s retry widening
    # below remains a reasonable safety margin for a from-source build on
    # a low-RAM VPS settling right after compiling, but it was never the
    # actual cause of the failures being investigated.
...
def _ensure_installed():
    ...
    build = _run(f"cd {build_dir} && cmake .. -DCMAKE_POLICY_VERSION_MINIMUM=3.5 -DBUILD_NOTHING_BY_DEFAULT=1 -DBUILD_UDPGW=1 && make install")
...
def _write_service(listen_port, max_clients, sndbuf=None, mtu=None):
    # Confirmed as a real, foundational diagnostic gap: badvpn-udpgw's own
    # stdout is buffered when not connected to a terminal - exactly the
    # case running under systemd - confirmed directly from a real
    # journalctl capture where its own "initializing" message didn't
    # appear until 25 minutes after the process actually started, only
    # flushing all at once when something later terminated it. This means
    # its own logs during any specific failed attempt were never actually
    # visible - they were sitting in a buffer, not reflecting what
    # genuinely happened at that moment. stdbuf -oL -eL forces line-
    # buffered output for any external program without needing to modify
    # its source, so its real, real-time behavior finally becomes visible
    # for diagnosing the actual "port never came up" failures, rather than
    # continuing to guess from misleadingly delayed output.
    exec_cmd = (
        f"/usr/bin/stdbuf -oL -eL {BADVPN_BIN} --listen-addr 127.0.0.1:{listen_port} "
...
Restart=always
RestartSec=3
# Confirmed as the real, direct explanation for a genuine bug report:
# badvpn-udpgw's own log shows a completely clean shutdown on SIGTERM
# ("termination requested" / "exiting"), yet it still exits with status 1,
# which systemd then logs as "Failed with result 'exit-code'" - a
# well-documented pattern for daemons that don't explicitly exit(0) in
# their own signal handler (confirmed against multiple independent, real
# examples of other services hitting the identical thing). Without this,
# EVERY restart - including this panel's own, and simple admin testing
# through it - counts as a "failure" toward systemd's restart-rate limit,
# even though the service was genuinely fine each time; enough of those in
# a short window and systemd just stops trying to bring it back at all,
# which is exactly "doesn't survive reboot until manually restarted".
# Telling systemd that exit code 1 means a normal, successful stop here -
# not a real failure - is the standard, correct fix for this exact class
# of quirk.
SuccessExitStatus=1
...
def _apply_settings_safely(listen_port, max_clients, description, sndbuf=None, mtu=None):
    ...
    if not _restart_badvpn():
        if original_unit is not None:
            ...
            # Hysteria2/MULTIPLEXING elsewhere in this panel: on a genuine
            # first-ever attempt (no prior working unit to revert to),
            # leaving the broken new unit sitting on disk - while telling
            # the admin it was "reverted" - meant every later attempt's
            # own rollback-on-failure would restore THIS same broken unit
            # as its own baseline, so every subsequent attempt failed
            # identically regardless of what was actually entered.
            try:
...
def badvpn_admin_manager(ports_dict):
    ...
    while True:
        ...
        if choice == '0':
            ...
            if str(listen_port) != str(badvpn_port) and check_system_port_in_use(listen_port, ("tcp",)):
                print(f"{C_RED}[X] TCP port {listen_port} is already in use by another service.{C_RESET}")
            ...
            if str(new_port) != str(badvpn_port) and check_system_port_in_use(new_port, ("tcp",)):
                print(f"{C_RED}[X] TCP port {new_port} is already in use by another service.{C_RESET}")
                input("\nPress Enter to continue...")
                continue
            ...
            if confirm == 'y':
                ...
                if str(badvpn_port).isdigit():
                    close_firewall_port(int(badvpn_port), ("tcp",))
                    persist_firewall_rules()
