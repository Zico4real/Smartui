...
def _wait_for_port_listening(port, tries=15, delay=1):
    # Widened from 6 tries (6s) to 15 tries (15s): confirmed via real,
    # unbuffered logs that the service itself binds and enters its event
    # loop correctly every single time with no error at all - the
    # verification step itself was the more likely bottleneck, especially
    # right after a from-source build on a VPS with limited RAM (confirmed
    # at 961Mi total), where the system may still be settling from that
    # build when this check runs immediately afterward.
