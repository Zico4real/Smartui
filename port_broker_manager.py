...
# Confirmed as the real, definitive root cause behind sslh always printing
# its own usage help and exiting with status 6/NOTCONFIGURED: the standard,
# unmodified Debian/Ubuntu sslh package's systemd unit reads its actual
# startup arguments from /etc/default/sslh's DAEMON_OPTS variable (via
# EnvironmentFile=) - confirmed against multiple independent, consistent
# sources. It never reads a /etc/sslh/sslh.cfg file at all unless the unit
# itself is separately modified to pass --config=, which the default
# package install does not do. Writing there meant sslh always started
# with the package's own unmodified placeholder DAEMON_OPTS (or none),
# explaining the printed usage text exactly.
SSLH_CONFIG_PATH = "/etc/default/sslh"
...
def _ensure_installed():
    ...
    # DEBIAN_FRONTEND=noninteractive is required here, not optional: sslh's
    # own Debian/Ubuntu package can raise a debconf prompt during install
    # (standalone vs inetd mode). Without a terminal to answer it - exactly
    # the case running as a subprocess from this panel - apt-get just hangs
    # indefinitely waiting for input that will never come, which matches a
    # real report of this install getting stuck for hours with zero
    # progress. This is the correct, standard fix for that entire class of
    # problem, not specific to sslh.
    #
    # DPkg::Lock::Timeout, confirmed as a second, separate real cause of the
    # identical symptom: this VPS runs unattended-upgrades and the daily apt
    # timers (confirmed directly from its own boot logs), and apt-get by
    # default waits FOREVER for the dpkg lock if either of those already
    # holds it when this runs - a well-documented, common pattern for
    # exactly this "stuck for hours, zero progress" report. Bounding the
    # wait to 60s means this either proceeds once the lock clears, or fails
    # cleanly with a real error the admin can see, instead of hanging
    # silently with no way to tell what it's even waiting on.
    _run("DEBIAN_FRONTEND=noninteractive apt-get -o DPkg::Lock::Timeout=60 update && "
         "DEBIAN_FRONTEND=noninteractive apt-get -o DPkg::Lock::Timeout=60 install -y sslh")
...
def _build_config(public_port, backends, has_openvpn_tcp):
    ...
    Builds /etc/default/sslh's DAEMON_OPTS as the actual command-line
    arguments the standard package's own unit expects (see the note on
    SSLH_CONFIG_PATH above) - not the libconfig .cfg file format sslh also
    supports but which the default unit never actually loads."""
    ...
    timeout = 5 if has_openvpn_tcp else 2
    # Confirmed as the real, direct cause of a genuine bug report, isolated
    # by a real, manual test on the actual VPS: this unit's own hardening
    # sets DynamicUser=true, which already creates and switches to its own
    # ephemeral, unprivileged user at startup - passing --user sslh on top
    # of that conflicts outright (sslh can't resolve a persistent "sslh"
    # system user the normal way under DynamicUser, and responds by
    # printing its own usage help and exiting NOTCONFIGURED, which is
    # exactly what a real, confirmed test showed). --pidfile is dropped
    # for the same reason, confirmed by the same real test producing a
    # "Permission denied" on that path next: DynamicUser's ephemeral user
    # has no write access there, and this unit declares no
    # RuntimeDirectory= to grant one - but a pidfile is a legacy mechanism
    # anyway, unneeded since systemd already tracks this process directly.
    opts = ["--listen 0.0.0.0:%s" % public_port, "--timeout %s" % timeout]
    if "ssh" in backends:
        opts.append("--ssh 127.0.0.1:%s" % backends["ssh"])
    if "openvpn" in backends:
        opts.append("--openvpn 127.0.0.1:%s" % backends["openvpn"])
    if "http" in backends:
        opts.append("--http 127.0.0.1:%s" % backends["http"])
    if "tls" in backends:
        # Must come last among these - TLS's probe is a broad "does this
        # look like a TLS handshake" check, so anything more specific
        # should be tried first.
        opts.append("--tls 127.0.0.1:%s" % backends["tls"])
    lines = [
        "# Managed by SmartUI panel - Port 443/80 Multiplexing module.",
        "RUN=yes",
        'DAEMON_OPTS="%s"' % " ".join(opts),
    ]
    return "\n".join(lines) + "\n"
...
def _apply_safely(public_port, backends, has_openvpn_tcp, description):
    ...
    if not _restart_sslh():
        if original is not None:
            ...
            # Hysteria2 elsewhere in this panel: on a genuine first-ever
            # attempt (no prior working config to revert to), leaving the
            # broken new config sitting on disk - while telling the admin
            # it was "reverted" - meant every later attempt's own
            # rollback-on-failure would restore THIS same broken config as
            # its own baseline, so every subsequent attempt failed
            # identically regardless of what was actually entered.
            try:
                os.remove(SSLH_CONFIG_PATH)
...
def port_broker_manager(ports_dict):
    ...
    while True:
        ...
        # UDP Droid/Checkuser elsewhere in this panel - confirmed as a real,
        # direct cause of a genuine bug report where this service installed
        # and configured correctly but was found "disabled" (never wired
        # to start at boot at all) after a reboot.
        if os.path.exists(SSLH_CONFIG_PATH):
