...
def sshgo_admin_manager(ports_dict):
    ...
    while True:
        ...
        print("----------------------------------------------------------------")
        print(" %s[1]>%s CONFIGURE / INSTALL SSHGO PROXY (Wizard)" % (C_YELLOW, C_RESET))
        print(" %s[2]>%s CHANGE PORT & TUNNEL MODE (SSH / WebSocket)" % (C_YELLOW, C_RESET))
        print(" %s[3]>%s VIEW SERVICE LOGS" % (C_YELLOW, C_RESET))
        print(" %s[4]>%s RESTART SSHGO SERVICE" % (C_YELLOW, C_RESET))
        print(" [5]> START/STOP SSHGO SERVICE [%s]" % status_label)
        print("%s════════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
        print(" %s[0]%s RETURN  %s[6]%s UNINSTALL SSHGO" % (C_YELLOW, C_RESET, C_YELLOW, C_RESET))
        print("%s════════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
