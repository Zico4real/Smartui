...
def _write_service(listen_port, target_port, path_prefix, use_tls=True):
    scheme = "wss" if use_tls else "ws"
    mask_flag = "" if use_tls else " --websocket-mask-frame"
    exec_cmd = ("%s server %s://0.0.0.0:%s --restrict-to 127.0.0.1:%s "
                "--restrict-http-upgrade-path-prefix %s%s") % (
        WSTUNNEL_BIN, scheme, listen_port, target_port, path_prefix, mask_flag)
...
def _apply_safely(listen_port, target_port, path_prefix, description, use_tls=True):
    original = None
    if os.path.exists(WSTUNNEL_SERVICE_PATH):
        with open(WSTUNNEL_SERVICE_PATH, "r") as f:
            original = f.read()

    _write_service(listen_port, target_port, path_prefix, use_tls)
...
def websocket_admin_manager(ports_dict):
    ...
    while True:
        ...
        if choice == '0':
            ...
            print("  [1] wss:// with TLS (default) - encrypted, but some ISPs/mobile")
            print("      carriers detect and silently drop WS/WSS traffic partway through")
            print("      the handshake, which looks like a plain connect timeout")
            print("  [2] ws:// plain, no TLS - if wss:// is timing out for your users,")
            print("      this removes the TLS handshake entirely and is worth trying")
            tls_choice = input(" Select [1-2] (default 1): ").strip()
            use_tls = tls_choice != '2'
            ...
            ok, msg = _apply_safely(listen_port, target_port, path_prefix, "WebSocket tunnel", use_tls)
            if ok:
                ...
                ports_dict['WSTUNNEL_USE_TLS'] = '1' if use_tls else '0'
                server_ip = get_public_ip()
                scheme = "wss" if use_tls else "ws"
                print("%s[OK] %s%s" % (C_GREEN, msg, C_RESET))
                print()
                print(" Client command:")
                print("  wstunnel client -L tcp://LOCAL_PORT:127.0.0.1:%s --http-upgrade-path-prefix %s %s://%s:%s" % (
                    target_port, path_prefix, scheme, server_ip, listen_port))
                print()
            ...
            use_tls = ports_dict.get('WSTUNNEL_USE_TLS', '1') != '0'
            open_firewall_port(new_port, ("tcp",))
            persist_firewall_rules()

            ok, msg = _apply_safely(new_port, new_target, path_prefix, "Port/redirect change", use_tls)
            if ok:
                old_port = ws_port
            ...
            print(" Mode: %s%s (%s)%s" % (
                C_YELLOW if not use_tls else C_GREEN,
                "TLS (wss)" if use_tls else "plain, no TLS (ws)",
                "default" if use_tls else "switched on manually - try this if wss:// times out",
                C_RESET))
            print()
            ...
            if use_tls:
                print()
                print("   %sIf connections get accepted then time out with no response" % C_YELLOW)
                print("   (check via option 4/journalctl: look for 'Doing TLS handshake'")
                print("   with nothing after it), that's a known pattern - some ISPs and")
                print("   mobile carriers detect and silently drop WS/WSS traffic mid-")
                print("   handshake. Try switching to plain mode (option 2, then pick")
                print("   'ws:// plain' when re-entering the port) if this happens.%s" % C_RESET)
            ...
            print("  wstunnel client -L tcp://LOCAL_PORT:127.0.0.1:%s --http-upgrade-path-prefix %s %s://%s:%s" % (
                ws_target, path_prefix, scheme, server_ip, ws_port))
            input("\nPress Enter to continue...")
