...
_RECONCILE_TARGETS = [
    ...
    ("UDPDROID_TUNNEL_PORT", "/etc/systemd/system/udp-droid-vpn.service", r'-p (\d+)'),
...
def _udp_droid_status(ports_dict):
    """Same shape of bug as ICMP's own fix above, caught before it ever
    shipped: the generic _status_tag() path checked a service literally
    named 'udp-droid' and a ports_dict key 'UDP_DROID_PORTS' - both
    leftover from the old UDP2RAW/port-scanning implementation this
    module replaced. The actual units this module's own admin screen
    creates are 'udp-droid-vpn' (the tunnel) and 'udp-droid-vpn-gate' (the
    gate), and the real port lives under 'UDPDROID_TUNNEL_PORT'. Checking
    the old names would always read OFF regardless of whether the new
    services were healthy - same "both together" requirement as ICMP's
    own check, since the tunnel is only genuinely usable once a customer
    can both unlock through the gate and reach the tunnel itself."""
    tunnel_active = _service_active("udp-droid-vpn")
    gate_active = _service_active("udp-droid-vpn-gate")
    if tunnel_active and gate_active:
        tunnel_port = ports_dict.get('UDPDROID_TUNNEL_PORT')
        return "%s[ON]%s" % (C_GREEN, C_RESET), tunnel_port
    return "%s[OFF]%s" % (C_RED, C_RESET), None
...
def protocol_configuration_manager(ports_dict):
    while True:
        ...
        entries = [
            ...
            ("23", "UDP DROID", None, None, None),  # handled specially below - two services, not a single generic port check
        ...
        for num, label, svc, port, proto in entries:
            if num == "1":
                ...
                tag, detail = _udp_droid_status(ports_dict)
            elif num == "24":
