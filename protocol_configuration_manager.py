...
def _websocket_status(ports_dict):
    """Same multi-instance pattern as Psiphon - multiple customer instances
    can exist under one gate service."""
    try:
        import websocket_manager as wsmod
        credentials = wsmod._load_credentials()
    except Exception:
        return "%s[OFF]%s" % (C_RED, C_RESET), None
    if not credentials:
        return "%s[OFF]%s" % (C_RED, C_RESET), None
    running = sum(1 for name in credentials if _service_active("wstunnel-%s" % name))
    if running == 0:
        return "%s[OFF]%s" % (C_RED, C_RESET), None
    return "%s[ON]%s" % (C_GREEN, C_RESET), "%d/%d running" % (running, len(credentials))
...
def protocol_configuration_manager(ports_dict):
    while True:
        ...
        entries = [
            ...
            ("8", "UDP-HYSTERIA", "hysteria-server", ports_dict.get('HYSTERIA_PORT'), "udp"),
            ...
            ("21", "WEBSOCKET (wstunnel)", "wstunnel-gate", ports_dict.get('WSTUNNEL_GATE_PORT'), "tcp"),
        ]
        ...
        for num, label, svc, port, proto in entries:
            if num == "3":
                ...
                tag, detail = _websocket_status(ports_dict)
            else:
        ...
        def print_row(n1, label1, n2, label2):
            print(" [%s]> %-20s%s   [%s]> %-20s%s" % (n1, label1, status_by_num[n1], n2, label2, status_by_num[n2]))
        ...
        print_row("8", "UDP-HYSTERIA", "17", "WIREGUARD")
        print_row("9", "BADVPN-UDPGW", "18", "ICMP TUNNEL")
        print(" [19]> PSIPHON (OSSH)   %s" % status_by_num["19"])
        print(" [20]> PORT 443/80 MULTIPLEX  %s" % status_by_num["20"])
        print(" [21]> WEBSOCKET (wstunnel)   %s" % status_by_num["21"])
