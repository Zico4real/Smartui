...
from panel_common import (
    ...
    close_firewall_port, persist_firewall_rules, check_system_port_in_use,
)
...
def extra_tools_manager(ports_dict):
    ...
    while True:
        ...
        print("%s════════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
        print(" [1]> SSL CERTIFICATE MANAGER")
        print(" [2]> SSH/DROPBEAR BANNER (use SSH Administrator instead)")
        print(" [3]> ROOT PASSWORD/ACCESS (use SSH Administrator instead)")
        print("----------------------------------------------------------------")
        print(" [4]> KERNEL ACCELERATION (BBR)")
        print("----------------------------------------------------------------")
        print(" [5]> BLOCKING TORRENT, PORTS AND KEYWORDS")
        print(" [6]> FAIL2BAN ADMINISTRATOR")
        print(" [7]> DDOS PROTECTION")
        print("----------------------------------------------------------------")
        print(" [8]> MODIFY TIME ZONE")
        print("----------------------------------------------------------------")
        print(" [9]> SPEEDTEST OOKLA")
        print(" [10]> PLUGINS SCRIPT")
        print("----------------------------------------------------------------")
        ...
        print("----------------------------------------------------------------")
        print(" [12]> RELEASE PORT (force-free a stuck port)")
        print("%s════════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
        print(" [0] Back")
        print("%s════════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
        ...
        if choice == '0':
            ...
            if not domain:
                print("%s[X] Domain cannot be empty.%s" % (C_RED, C_RESET))
                input("\nPress Enter to continue...")
                continue
            email = input(" Enter your email address (blank to skip): ").strip()
            ...
            if email:
                cert_cmd = ["certbot", "certonly", "--standalone", "--agree-tos",
                            "-m", email, "-d", domain, "--force-renewal", "-n"]
            else:
                cert_cmd = ["certbot", "certonly", "--standalone", "--agree-tos",
                            "--register-unsafely-without-email", "-d", domain, "--force-renewal", "-n"]
            ...
            if result.returncode == 0:
                # Same pattern Xray's own cert generator uses: without this, a
                # renewal 60-90 days later updates the raw /etc/letsencrypt
                # files but nothing that actually reads a COPY of the cert
                # from somewhere else (or needs a restart to pick it up) ever
                # sees the new one - it silently keeps using the expiring
                # cert until something breaks. Restarting any of the known
                # cert-consuming services that are actually active covers
                # this without needing to ask which one the cert is for.
                hook_dir = "/etc/letsencrypt/renewal-hooks/deploy"
                os.makedirs(hook_dir, exist_ok=True)
                hook_path = "%s/panel-ssl-renew-%s.sh" % (hook_dir, domain.replace(".", "-"))
                hook_script = (
                    "#!/bin/bash\n"
                    "for svc in stunnel4 xray nginx apache2; do\n"
                ...
                print("%s[i] Auto-renewal deploy hook installed at %s -" % (C_CYAN, hook_path))
                print("    future renewals will restart whichever of stunnel4/xray/nginx/apache2")
                print("    is actually running, so a renewed cert doesn't silently go unused.%s" % C_RESET)
            else:
...
"""
            ...
            if fw_backend:
                ...
                if confirm == "YES":
                    if fw_backend == "ufw":
                        ...
                        # iptables has no "service" to stop on most modern
                        # systems (no such unit exists unless iptables-persistent
                        # is in play) - flushing the actual rules and resetting
                        # the default policies is what genuinely disables it,
                        # matching how detect_firewall() itself now determines
                        # "active" (real rules or a DROP/REJECT policy, not
                        # just the binary's presence).
                        _run(["iptables", "-F"])
                        _run(["iptables", "-X"])
                        _run(["iptables", "-P", "INPUT", "ACCEPT"])
                        _run(["iptables", "-P", "FORWARD", "ACCEPT"])
                        result = _run(["iptables", "-P", "OUTPUT", "ACCEPT"])
                    if result.returncode == 0:
            ...
            clear_screen()
            print("%s════════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
            print("%s                       RELEASE PORT                         %s" % (C_BOLD, C_RESET))
            print("%s════════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
            print(" Frees a stuck port and clears any firewall rule for it - use this")
            print(" when a port stays bound after removing or uninstalling something")
            print(" that used to listen on it.")
            print()
            port_raw = input(" Which port do you want to release? ").strip()
            if not port_raw.isdigit():
                print("%s[X] Enter a plain port number.%s" % (C_RED, C_RESET))
                input("\nPress Enter to continue...")
                continue
            port = int(port_raw)

            tcp_in_use = check_system_port_in_use(port, ("tcp",))
            udp_in_use = check_system_port_in_use(port, ("udp",))

            if not tcp_in_use and not udp_in_use:
                print("%s[i] Nothing is currently bound to port %d." % (C_CYAN, port))
                print("    Clearing any leftover firewall rule for it anyway.%s" % C_RESET)
            else:
                print(" Currently bound as: %s%s%s" % (
                    C_YELLOW,
                    ", ".join(p for p, used in (("tcp", tcp_in_use), ("udp", udp_in_use)) if used),
                    C_RESET))
                lookup = _run(["fuser", "%d/tcp" % port, "%d/udp" % port])
                pids = lookup.stdout.split() if lookup.stdout else []
                # Confirmed still recurring after the first fix: killing the
                # raw PID alone isn't enough when it's managed by a systemd
                # service with Restart=always - the service just relaunches
                # with a brand new PID. The first fix tried mapping PID back
                # to its unit by parsing `systemctl status <PID>`'s
                # human-oriented text output, which is not a stable,
                # machine-readable format (varies by systemd version,
                # terminal width, locale) - if it failed to match for any
                # reason, this fell through to just killing the raw PID
                # again, silently reproducing the exact bug being fixed.
                # Layering three independent checks instead: (1) this
                # panel's own known, predictable service-naming convention
                # for the exact port in question (python.<port>.service -
                # zero ambiguity, since this panel itself controls that
                # name), (2) /proc/<pid>/cgroup, a stable kernel interface
                # that names the owning unit directly rather than a
                # human-oriented CLI report, and (3) `systemctl status
                # <PID>` parsing as a last-resort fallback. Any one hit is
                # enough to stop+disable the real unit instead of just
                # killing a PID that comes back.
                units_to_disable = set()
                known_pattern_unit = "python.%d.service" % port
                if os.path.exists("/etc/systemd/system/%s" % known_pattern_unit):
                    units_to_disable.add(known_pattern_unit)
                    print("   Known service for this port: %s" % known_pattern_unit)
                for pid in pids:
                    name_res = _run(["ps", "-p", pid, "-o", "comm="])
                    name = name_res.stdout.strip() if name_res.returncode == 0 else "unknown"
                    unit = None
                    try:
                        with open("/proc/%s/cgroup" % pid) as f:
                            cgroup_content = f.read()
                        m = re.search(r'/([^/]+\.service)\s*$', cgroup_content, re.MULTILINE)
                        if m:
                            unit = m.group(1)
                    except (FileNotFoundError, PermissionError):
                        pass
                    if not unit:
                        status_res = _run(["systemctl", "status", pid])
                        m = re.search(r'^\S*\s*(\S+\.service)', status_res.stdout, re.MULTILINE)
                        if m:
                            unit = m.group(1)
                    if unit:
                        units_to_disable.add(unit)
                        print("   PID %s (%s) - managed by systemd unit: %s" % (pid, name, unit))
                    else:
                        print("   PID %s (%s) - not managed by systemd" % (pid, name))

                confirm = input(" Free port %d - stop/disable any owning service and kill any remaining process? (y/n): " % port).strip().lower()
                if confirm != 'y':
                    print("%s[i] Cancelled - nothing was changed.%s" % (C_YELLOW, C_RESET))
                    input("\nPress Enter to continue...")
                    continue

                for unit in units_to_disable:
                    stop_res = _run(["systemctl", "stop", unit])
                    disable_res = _run(["systemctl", "disable", unit])
                    if stop_res.returncode != 0:
                        print("   %s[!] 'systemctl stop %s' returned an error - see below.%s" % (C_YELLOW, unit, C_RESET))
                        if stop_res.stderr:
                            print("   %s" % stop_res.stderr.strip())
                _run(["fuser", "-k", "%d/tcp" % port])
                _run(["fuser", "-k", "%d/udp" % port])

            close_firewall_port(port, ("tcp", "udp"))
            persist_firewall_rules()

            still_used = check_system_port_in_use(port, ("tcp",)) or check_system_port_in_use(port, ("udp",))
            if still_used:
                print("%s[!] Port %d still shows as bound after stopping the owning service" % (C_YELLOW, port))
                print("    and killing the process - something else may also be using it, or")
                print("    a second instance is respawning it from elsewhere.%s" % C_RESET)
            else:
                print("%s[OK] Port %d is now free and its firewall rule has been cleared.%s" % (C_GREEN, port, C_RESET))
            input("\nPress Enter to continue...")
