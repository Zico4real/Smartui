...
"""
            ...
            print("%s════════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
            print(" Forcibly kills whatever process is holding a port and clears any")
            print(" firewall rule for it - use this when a port stays stuck after")
            print(" removing or uninstalling something that used to listen on it.")
            print()
            ...
            if not tcp_in_use and not udp_in_use:
                ...
                if pids:
                    for pid in pids:
                        ...
                        print("   PID %s (%s)" % (pid, name))
                confirm = input(" Kill whatever's using port %d and clear its firewall rule? (y/n): " % port).strip().lower()
                if confirm != 'y':
            ...
            if still_used:
                print("%s[!] Port %d still shows as bound after the kill attempt - it may be" % (C_YELLOW, port))
                print("    respawning via a systemd service with Restart=always. Stop and")
                print("    disable that service directly rather than just killing the process.%s" % C_RESET)
            else:
