...
# Confirmed as the real, definitive root cause behind every single
# Hysteria2 "port never came up" report throughout this whole project:
# the actual, real installed unit's ExecStart (confirmed directly from a
# real cat of /etc/systemd/system/hysteria-server.service) points at
# config.yaml - this was writing to config.json instead, a file Hysteria2
# never actually reads at all. Every setting ever entered here - port,
# password, OBFS, ACME domain - never reached the real running service;
# it was always loading whatever the install script's own default
# config.yaml already contained. The content written is still valid JSON,
# which is fine here since JSON is a strict subset of YAML and Go's YAML
# libraries (what Hysteria2 itself uses) parse it correctly either way -
# only the file path itself needed to change.
HYSTERIA_CONFIG_PATH = f"{HYSTERIA_DIR}/config.yaml"
...
def _generate_self_signed_cert():
    ...
    if ok:
        ...
        # the same class of issue already found and fixed for Xray's own
        # TLS key elsewhere in this panel: openssl's -keyout defaults to
        # restrictive permissions (typically 600, owner-only), and since
        # this runs as root, the key ends up readable only by root.
        # Hysteria2 itself runs as a separate, dedicated "hysteria" user
        # (confirmed from its own unit file's User=/Group=), so it could
        # never actually read its own key - confirmed directly from a
        # real "permission denied" error on this exact file. _ensure_installed()
        # already ran by this point in the setup wizard, so the official
        # install script's own "hysteria" user is guaranteed to already
        # exist here.
        _run(f"chown hysteria:hysteria {key_path} {cert_path}")
...
def _apply_config_safely(new_config, description, check_port):
    ...
    if not _restart_hysteria():
        if original is not None:
            ...
            # Confirmed as a real, severe bug: on a genuine first-ever
            # attempt (no prior working config exists to revert to), the
            # broken new config was left sitting on disk - while the
            # message shown to the admin unconditionally claimed it was
            # "reverted, nothing was left broken". Every later attempt's
            # own rollback-on-failure would then restore THIS same broken
            # config as its own "previous working" baseline, meaning every
            # subsequent attempt failed the exact same way regardless of
            # what was actually entered, with no way to tell from the
            # panel alone. Removing the file entirely restores the
            # genuine "not configured yet" state instead of leaving a
            # silently broken one in its place.
            try:
                os.remove(HYSTERIA_CONFIG_PATH)
...
def hysteria_admin_manager(ports_dict):
    ...
    while True:
        ...
        if choice == '0':
            ...
            email = ""
            existing_cert, existing_key = None, None
            if domain:
                # Confirmed as a real, worthwhile improvement: certbot (used by
                # both Xray's and Stunnel's own cert flows elsewhere in this
                # panel) always stores its certs at this same standard path
                # regardless of which module originally requested them - so a
                # domain already set up for either of those can be detected and
                # reused directly here, skipping Hysteria2's own separate ACME
                # request entirely. This also sidesteps a real, confirmed
                # requirement of Hysteria2's own ACME client: it needs port 80
                # reachable for its HTTP-01 challenge (Hysteria2's own official
                # install guidance says this explicitly), which this panel's
                # install flow never opened or checked for - reusing an
                # existing cert avoids ever needing that challenge to begin
                # with, for the common case of a domain already used elsewhere.
                candidate_cert = "/etc/letsencrypt/live/%s/fullchain.pem" % domain
                candidate_key = "/etc/letsencrypt/live/%s/privkey.pem" % domain
                if os.path.exists(candidate_cert) and os.path.exists(candidate_key):
                    print(f"{C_GREEN}[✔] Found an existing certificate for {domain} (already set up")
                    print(f"    elsewhere in this panel) - reusing it directly, no new ACME request needed.{C_RESET}")
                    existing_cert, existing_key = candidate_cert, candidate_key
                else:
            ...
            if existing_cert and existing_key:
                config_data["tls"] = {"cert": existing_cert, "key": existing_key}
            elif domain and email:
                # Genuine fallback to Hysteria2's own ACME - port 80 must be
                # reachable for its HTTP-01 challenge (confirmed directly from
                # Hysteria2's own official install guidance), which nothing
                # in this flow ever opened before this fix.
            ...
            if obfs_pass:
                # Confirmed as the real, definitive root cause of every
                # "PSK must be at least 4 bytes" failure: the correct,
                # official YAML structure nests the password under
                # obfs.salamander.password, not obfs.password directly -
                # confirmed identically across multiple independent
                # official Hysteria2 doc pages. Writing it at the wrong
                # level meant the real path was always empty, so Hysteria2
                # always saw a 0-byte PSK regardless of what was actually
                # typed here.
                config_data["obfs"] = {"type": "salamander", "salamander": {"password": obfs_pass}}
            ...
            if new_obfs.lower() == 'none':
                ...
                # Same fix as the identical bug in the setup wizard above -
                # the password must nest under obfs.salamander.password,
                # not obfs.password directly.
                cfg["obfs"] = {"type": "salamander", "salamander": {"password": new_obfs}}
