...
def atken_hash_admin_manager(ports_dict):
    ...
    while True:
        ...
        print("----------------------------------------------------------------")
        print(" %s[1]>%s CONFIGURE / INSTALL ATKEN HASH SERVICE (Wizard)" % (C_YELLOW, C_RESET))
        print(" %s[2]>%s ADD / GENERATE TOKEN" % (C_YELLOW, C_RESET))
        print(" %s[3]>%s REVOKE A TOKEN" % (C_YELLOW, C_RESET))
        print(" %s[4]>%s CHANGE TOKEN PORT" % (C_YELLOW, C_RESET))
        print(" %s[5]>%s VIEW ATKEN / HASH SERVICE LOGS" % (C_YELLOW, C_RESET))
        print(" %s[6]>%s RESTART ATKEN HASH SERVICE" % (C_YELLOW, C_RESET))
        print(" [7]> START/STOP ATKEN SERVICE [%s]" % status_label)
        print(" %s[9]>%s VIEW / RESET DEVICE BINDINGS (HWID)" % (C_YELLOW, C_RESET))
        print("%s════════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
        print(" %s[0]%s RETURN  %s[8]%s UNINSTALL ATKEN HASH" % (C_YELLOW, C_RESET, C_YELLOW, C_RESET))
        print("%s════════════════════════════════════════════════════════════════%s" % (C_CYAN, C_RESET))
