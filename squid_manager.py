...
def squid_admin_manager(ports_dict):
    ...
    while True:
        ...
        if choice == '0':
            ...
            # Automatic, exact-match reuse - confirmed as the real,
            # actionable fix for a genuine scaling problem: even typing
            # part of a username to search doesn't work when an admin
            # genuinely has thousands of real customers. The username
            # typed above IS the lookup key - no separate search prompt,
            # no disambiguation list, ever.
            existing_ssh_users = {}
            try:
            ...
            username = input(" Enter the customer's username (reused automatically if it matches\n"
                              " an existing SSH account, otherwise a new one is created): ").strip()
            if not username:
                print("%s[X] A username is required.%s" % (C_RED, C_RESET))
                input("\nPress Enter to continue...")
                continue

            if username in existing_ssh_users:
                ...
                print("%s[i] No existing SSH account named '%s' - creating a new, separate" % (C_YELLOW, username))
                print("    proxy-only login. Generated password: %s%s" % (password, C_RESET))
            ...
            if sub == '1':
                ...
                if not username:
                    ...
                    # Same automatic, exact-match SSH account reuse as the
                    # install wizard above - this is the flow that
                    # actually runs once per new customer, so it's where
                    # the real scaling concern lives.
                    existing_ssh_users = {}
                    try:
                        from ssh_user_manager import _load_registry as _load_ssh_registry, _load_password_store as _load_ssh_passwords
                        ssh_registry = _load_ssh_registry()
