...
def squid_admin_manager(ports_dict):
    ...
    while True:
        ...
        if choice == '0':
            ...
            # Same fix as Psiphon's own instance setup elsewhere in this
            # panel: let the admin reuse an existing SSH account's real
            # credentials, rather than always typing a brand-new, separate
            # username/password just for proxy access.
            existing_ssh_users = {}
            try:
            ...
            if existing_ssh_users:
                # Confirmed as a real, necessary fix to a genuine scaling
                # concern: dumping every single SSH account into a numbered
                # list works fine with a handful of accounts, but becomes
                # completely unusable - scrolling through hundreds of
                # entries to find one - for a reseller with many real
                # customers. Searching by typing part of the username
                # instead, the same way the admin would think to look
                # someone up, scales to any number of accounts without
                # ever dumping the full list unless there's a genuine need
                # to disambiguate a few close matches.
                search = input(" Type part of the customer's username to reuse their SSH account\n"
                                " (blank to create a separate, new username/password instead): ").strip()
                if search:
                    matches = sorted(u for u in existing_ssh_users if search.lower() in u.lower())
                    if not matches:
                        print("%s[i] No SSH account matches '%s' - creating a separate, new one instead.%s" % (C_YELLOW, search, C_RESET))
                    elif len(matches) == 1:
                        username = matches[0]
                        password = existing_ssh_users[username]
                        print("%s[i] Reusing SSH account '%s' - same username and password will" % (C_CYAN, username))
                        print("    unlock both SSH and this proxy.%s" % C_RESET)
                    else:
                        print(" Multiple accounts match '%s':" % search)
                        for i, u in enumerate(matches, 1):
                            print("   [%d] %s" % (i, u))
                        print("   [0] None of these - create a separate, new username/password instead")
                        sel = input(" Which one (number): ").strip()
                        if sel.isdigit() and 1 <= int(sel) <= len(matches):
                            username = matches[int(sel) - 1]
                            password = existing_ssh_users[username]
                            print("%s[i] Reusing SSH account '%s' - same username and password will" % (C_CYAN, username))
                            print("    unlock both SSH and this proxy.%s" % C_RESET)
