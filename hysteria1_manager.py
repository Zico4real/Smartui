...
def _enable_udp_mod(real_port, ports_dict=None):
    ...
    if ports_dict:
        ...
        # UDP_DROID_PORTS, which is a genuine comma-separated LIST since
        # UDP Droid explicitly supports multiple ports for client apps that
        # scan for an open one - handling only a single int would have
        # missed every port beyond the first anyway). Verified each of
        # these five key names directly against where its own module
        # actually sets it, rather than assuming.
        for key in ('ZIVPN_PORT', 'BADVPN_PORT', 'ICMP_GATE_PORT', 'HYSTERIA_PORT'):
            val = ports_dict.get(key)
            if val and str(val).isdigit() and int(val) != real_port:
                exclude_ports.add(int(val))
        udp_droid_raw = ports_dict.get('UDP_DROID_PORTS', '')
        for p in udp_droid_raw.split(','):
            if p.isdigit() and int(p) != real_port:
                exclude_ports.add(int(p))
